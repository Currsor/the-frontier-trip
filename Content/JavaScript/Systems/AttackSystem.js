...
class AttackSystem {
    ...
    lastAttackTime = 0;
    attackComboCount = 0;
    attackComboResetTime = 2000; // 2秒后重置连击
    static getInstance() {
    ...
    constructor() {
        this.gameLogicManager = GameLogicManager_1.GameLogicManager.getInstance();
        this.stateManager = StateManager_1.StateManager.getInstance();
     ...
     */
    setupEventListeners() {
        EventSystem_1.EventSystem.subscribe("onAttackInput", (data) => {
            this.handleAttackInput(data.attacker);
        });
        EventSystem_1.EventSystem.subscribe("onAttackHitboxActivated", (data) => {
            this.handleAttackHitboxActivation(data.attacker);
        });
        EventSystem_1.EventSystem.subscribe("onAttackEnd", (data) => {
    ...
    handleAttackInput(attacker) {
        ...
        if (currentTime - this.lastAttackTime < GameConfig_1.GameConfig.ATTACK_COOLDOWN * 1000) {
    ...
    canStartAttack(attacker) {
        if (!attacker || !attacker.IsValidLowLevel()) {
            return false;
        }
        // 检查状态是否允许攻击
        ...
        const targetState = this.stateManager.isMoving() ?
            StateManager_1.StateManager.STATES.RUN_ATTACK : StateManager_1.StateManager.STATES.ATTACK;
    ...
    startAttack(attacker) {
        ...
        // 广播攻击开始事件
        EventSystem_1.EventSystem.emit("onAttackStarted", {
            attacker: attacker,
     ...
     * @param attacker 攻击者
     */
    handleAttackHitboxActivation(attacker) {
        console.log(`Attack hitbox activated for ${attacker.GetName()}`);
        // 广播碰撞盒激活事件
        EventSystem_1.EventSystem.emit("onAttackHitboxActive", {
            attacker: attacker,
            timestamp: Date.now()
        });
    }
    ...
    processAttackHit(attacker, target, hitResult) {
        ...
        const finalDamage = this.gameLogicManager.calculateDamage(baseDamage, this.getAttackerLevel(attacker), this.getWeaponMultiplier(attacker), isCritical);
        // 应用伤害
    ...
    calculateBaseDamage(attacker) {
        ...
        // 攻击类型加成
        if (this.stateManager.getCurrentState() === StateManager_1.StateManager.STATES.RUN_ATTACK) {
     ...
     * @param hitResult 命中结果
     */
    applyDamage(target, damage, instigator, hitResult) {
     ...
     */
    handleAttackFeedback(attacker, target, damage, isCritical) {
        // 屏幕震动
        if (GameConfig_1.GameConfig.EFFECT_CONFIG.SCREEN_SHAKE_INTENSITY > 0) {
            EventSystem_1.EventSystem.emit("onScreenShake", {
                intensity: GameConfig_1.GameConfig.EFFECT_CONFIG.SCREEN_SHAKE_INTENSITY,
                duration: 0.2
            });
        }
        // 伤害数字显示
        if (GameConfig_1.GameConfig.DEBUG_CONFIG.SHOW_DAMAGE_NUMBERS) {
            EventSystem_1.EventSystem.emit("onShowDamageNumber", {
                target: target,
                damage: damage,
        ...
        // 音效播放
        EventSystem_1.EventSystem.emit("onPlayAttackSound", {
            attacker: attacker,
            target: target,
            isCritical: isCritical
        });
        // 粒子效果
        EventSystem_1.EventSystem.emit("onPlayAttackEffect", {
            attacker: attacker,
            target: target,
    ...
    handleAttackEnd(attacker) {
        ...
        // 广播攻击结束事件
        EventSystem_1.EventSystem.emit("onAttackCompleted", {
            attacker: attacker,
            comboCount: this.attackComboCount,
            timestamp: Date.now()
        });
