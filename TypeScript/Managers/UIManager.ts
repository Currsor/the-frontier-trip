...
export class UIManager {
    ...
    private healthBarUpdateSpeed: number = GameConfig.UI_CONFIG.HEALTH_BAR_UPDATE_SPEED;
    private damageNumberPool: Array<any> = [];
    private activeNotifications: Array<any> = [];
    ...
    private setupEventListeners(): void {
        EventSystem.subscribe("onHealthChanged", (data: any) => {
            this.updateHealthBar(data.currentHealth, data.maxHealth, data.target);
        });

        EventSystem.subscribe("onShowDamageNumber", (data: any) => {
        ...
        EventSystem.subscribe("onExpGained", (data: any) => {
     ...
     * 初始化伤害数字对象池
     */
    private initializeDamageNumberPool(): void {
        for (let i = 0; i < GameConfig.PERFORMANCE_CONFIG.MAX_DAMAGE_NUMBERS; i++) {
     ...
     * @param target 目标角色
     */
    public updateHealthBar(currentHealth: number, maxHealth: number, target?: Actor): void {
        ...
        // 广播UI更新事件给蓝图
        EventSystem.emit("onUpdateHealthBarUI", {
     ...
     */
    public showDamageNumber(damage: number, position: any, isCritical: boolean = false): void {
        ...
        // 广播伤害数字显示事件
        EventSystem.emit("onDisplayDamageNumber", {
            damage: damage,
            position: position,
            isCritical: isCritical,
            duration: GameConfig.UI_CONFIG.DAMAGE_TEXT_FADE_TIME,
        ...
        }, GameConfig.UI_CONFIG.DAMAGE_TEXT_FADE_TIME * 1000);
     ...
     * @returns 伤害数字对象
     */
    private getDamageNumberFromPool(): any {
     ...
     */
    private returnDamageNumberToPool(damageNumber: any): void {
     ...
     * @param oldState 旧状态
     */
    public updateStateDisplay(newState: string, oldState: string): void {
        console.log(`State display updated: ${oldState} -> ${newState}`);

        // 广播状态UI更新事件
        EventSystem.emit("onUpdateStateUI", {
            newState: newState,
            oldState: oldState,
            timestamp: Date.now()
        });
     ...
     * @param isCritical 是否暴击
     */
    public showHitEffect(target: Actor, isCritical: boolean): void {
        // 屏幕闪烁效果
        EventSystem.emit("onScreenFlash", {
        ...
        // 击中标记
        EventSystem.emit("onShowHitMarker", {
            target: target,
            isCritical: isCritical,
            duration: 0.5
        });
     ...
     * @param items 掉落物品列表
     */
    public showLootNotification(items: string[]): void {
        ...
        const notification = {
            ...
            items: items,
            startTime: Date.now(),
            duration: GameConfig.UI_CONFIG.NOTIFICATION_DISPLAY_TIME
        };

        this.activeNotifications.push(notification);

        // 广播掉落通知事件
        EventSystem.emit("onShowLootNotification", {
            items: items,
     ...
     */
    public updateExpBar(currentExp: number, maxExp: number, gainedExp: number): void {
        const expPercentage = currentExp / maxExp;

        console.log(`Updating exp bar: ${currentExp}/${maxExp} (+${gainedExp})`);

        // 广播经验值UI更新事件
        EventSystem.emit("onUpdateExpBarUI", {
     ...
     * 显示低生命值警告
     */
    private showLowHealthWarning(): void {
        EventSystem.emit("onShowLowHealthWarning", {
            timestamp: Date.now()
        });
    }
    ...
    private showDeathScreen(): void {
        EventSystem.emit("onShowDeathScreen", {
            timestamp: Date.now()
        });
    ...
    private showHurtEffect(): void {
        EventSystem.emit("onShowHurtEffect", {
            duration: 0.5,
            timestamp: Date.now()
        });
    }
    ...
    private showAttackIndicator(): void {
        EventSystem.emit("onShowAttackIndicator", {
            duration: 0.3,
            timestamp: Date.now()
        });
    }
    ...
    private showLevelUpEffect(): void {
        EventSystem.emit("onShowLevelUpEffect", {
            timestamp: Date.now()
        });
    ...
    private removeNotification(notificationId: number): void {
        ...
        if (index > -1) {
            ...
            EventSystem.emit("onRemoveNotification", {
     ...
     * @param info 调试信息
     */
    public showDebugInfo(info: any): void {
        if (!GameConfig.DEBUG_CONFIG.SHOW_STATE_TRANSITIONS) {
            return;
        }

        EventSystem.emit("onShowDebugInfo", {
            info: info,
            timestamp: Date.now()
        });
    }
     ...
     * @param config 配置对象
     */
    public updateUIConfig(config: {
        healthBarUpdateSpeed?: number;
        damageTextFadeTime?: number;
