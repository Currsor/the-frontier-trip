...
export class GameConfig {
    // 伤害相关配置
    static readonly DAMAGE_MULTIPLIERS = {
        NORMAL: 1.0,
        CRITICAL: 2.0,
        WEAK: 0.5,
        SUPER_EFFECTIVE: 1.5
    ...
    static readonly MIN_DAMAGE = 1;
    static readonly MAX_HEALTH = 1000;
    static readonly LEVEL_DAMAGE_MULTIPLIER = 0.1; // 每级增加10%伤害

    // 攻击系统配置
    static readonly ATTACK_COOLDOWN = 0.5; // 秒
    static readonly DEFAULT_ATTACK_RANGE = 200; // 单位
    static readonly CRITICAL_HIT_BASE_CHANCE = 0.1; // 10%基础暴击率

    // 物品掉落配置
    static readonly ITEM_DROP_RATES = {
        COMMON: 0.7,    // 70%
        UNCOMMON: 0.2,  // 20%
        RARE: 0.08,     // 8%
        EPIC: 0.02      // 2%
    ...
    static readonly DROP_RADIUS = 100; // 掉落物品散布半径

    // 状态系统配置
    static readonly STATE_PRIORITIES = {
        Dead: 100,
        Hurt: 90,
        Dash: 80,
        Attack: 70,
    ...
    // 移动相关配置
    static readonly MOVEMENT_THRESHOLDS = {
        WALK_THRESHOLD: 10.0,
        RUN_THRESHOLD: 700.0,
     ...
     */
    public static getStatePriority(state: string): number {
     ...
     * @returns 是否可以转换
     */
    static canTransitionState(fromState: string, toState: string): boolean {
     ...
     * @returns 掉落概率
     */
    public static getDropRate(rarity: string): number {
        return (this.ITEM_DROP_RATES as any)[rarity.toUpperCase()] || 0;
    }
     ...
     * @returns 是否有效
     */
    static validateConfig(configKey: string, value: any): boolean {
        switch (configKey) {
            case "health":
