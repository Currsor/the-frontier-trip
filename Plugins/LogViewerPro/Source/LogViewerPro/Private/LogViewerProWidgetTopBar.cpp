...
void SLogViewerProWidgetTopBar::Construct(const FArguments& InArgs)
{
	...
	FileMenu = SNew(SLogViewerProFileMenu).MainWidget(MainWidget);
					...
					SNew(SHorizontalBox)
						...
						[
							SNew(SCheckBox)
								...
								.IsChecked_Lambda([this]()->ECheckBoxState { return this->MainWidget->IsNamedTabDirty() ? ECheckBoxState::Checked : ECheckBoxState::Unchecked; })
								.OnCheckStateChanged(this, &SLogViewerProWidgetTopBar::HandleResaveTabPressed)
								.Visibility_Lambda([this]()->EVisibility
									{
										return this->MainWidget->IsNamedTab() ? EVisibility::Visible : EVisibility::Collapsed;
									})
								[
									SNew(SBox)
										...
										[
											SNew(STextBlock)
												.Text(LOCTEXT("Resave Tab", "Resave Tab"))
										]
								]
						]
									...
									SNew(SBox)
										...
										[
											SAssignNew(ClearOutputCaption, STextBlock)
												.Text_Lambda([&]() { return MainWidget->LogMessages.Num() > 0 ? LOCTEXT("LogViewerProClearLog", "Clear Log") : LOCTEXT("LogViewerProClearOutput", "Clear Categories"); })
										]
								]
						]
...
void SLogViewerProWidgetTopBar::HandleResaveTabPressed(ECheckBoxState CheckState)
{
	if (!MainWidget->IsNamedTab())
	{
		UE_LOG(LogTemp, Error, TEXT("Log Viewer: Action ignored: Attempt to resave named tab, but can't retrieve an actual tab name. If you see that issue (you shouldn't), please reach me on Discord. Thanks!"));
		return;
	}

	MainWidget->ResaveNamedTab();
}
...
void SLogViewerProWidgetTopBar::OpenLog()
{
	...
	if (DesktopPlatform)
	{
		...
		bOpened = DesktopPlatform->OpenFileDialog(
			...
			LOCTEXT("OpenProjectBrowseTitle", "Open Log File").ToString(),
	...
	{
		MainWidget->GetParentTab().Pin()->SetLabel(FText::FromString(FPaths::GetCleanFilename(Path)));
	}
