...
void SLogViewerProWidgetCategoriesView::Construct(const FArguments& InArgs)
{
	...
	ChildSlot
		[
			SNew(SBorder)
				.Padding(0)
				.Visibility_Lambda([this]() { return MainWidget->GetSettings()->IsMinimizeCategories() ? EVisibility::Collapsed : EVisibility::Visible; })
												...
												[
													SNew(SBox)
														...
														[
															SNew(STextBlock)
																.TextStyle(FAppStyle::Get(), "SmallText")
																.Text_Lambda([&]() { return MainWidget->GetLogViewerProModule()->IsDefaultCategoriesEnabled() ? LOCTEXT("LogViewerProClearDefault", "Default") : LOCTEXT("LogViewerProMakeDefault", "Default"); })
...
void SLogViewerProWidgetCategoriesView::MinimizeCategories()
{
...
void SLogViewerProWidgetCategoriesView::LoadCategoriesFromFile(const FString& Path)
{
	FLVCategoriesSave CategoriesSave;
	if (!ParseCategoriesFile(Path, CategoriesSave))
	{
		...
		{
			FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("SaveLogDialogFileError", "Unable to parse categories file. Something is wrong, maybe file format. File name is available in the logs."));
			UE_LOG(LogTemp, Error, TEXT("LogViewer: Unable to parse file: %s. If issue persists, please reach me on Discord."), *Path);
			return;
		}
	}
	ApplyCategoriesSave(CategoriesSave);
