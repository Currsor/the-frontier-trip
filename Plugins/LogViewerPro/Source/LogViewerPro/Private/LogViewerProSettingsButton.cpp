...
TSharedRef<SWidget> SLogViewerProSettingsButton::GenerateSettingsMenu()
{
	...
	MenuBuilder.BeginSection("Behavior", LOCTEXT("LVSettingsBehavior", "Behavior"));
	{
		...
		MenuBuilder.AddMenuEntry(
			LOCTEXT("LVPSettingsMinimizeCategory", "Minimize Categories"),
			LOCTEXT("LVPSettingsMinimizeCategoryTooltip", "Minimize categories panel to gain more horizontal space for logs"),
			FSlateIcon(),
			FUIAction(
				FExecuteAction::CreateSP(this, &SLogViewerProSettingsButton::ToggleMinimizeCategories),
				FCanExecuteAction(),
				FIsActionChecked::CreateSP(this, &SLogViewerProSettingsButton::IsMinimizeCategories)
			),
			NAME_None,
			EUserInterfaceActionType::ToggleButton
		);
...
void SLogViewerProSettingsButton::ToggleMinimizeCategories()
{
	SettingsData.bMinimizeCategories = !SettingsData.bMinimizeCategories;
	MainWidget->Invalidate(EInvalidateWidgetReason::Layout);
	MainWidget->CategoryMenu->Invalidate(EInvalidateWidgetReason::Layout);
	//MainWidget->CategoryMenu->MinimizeCategories();
...
bool SLogViewerProSettingsButton::IsMinimizeCategories() const
{
	return SettingsData.bMinimizeCategories;
}
...
FString SLogViewerProSettingsButton::GetDefaultNamedTabsFolderPath()
{
