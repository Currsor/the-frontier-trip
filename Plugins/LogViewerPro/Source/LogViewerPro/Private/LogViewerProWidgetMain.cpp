					...
					SNew(SVerticalBox)
						...
						[
							SNew(SHorizontalBox)
								...
								+ SHorizontalBox::Slot()
								.AutoWidth()
								//.WidthOverride_Lambda([this]() {return GetSettings()->IsMinimizeCategories() ? 20.f : 300.f; })
								//.FillWidth(bCategoryMenuMinimized ? 0.f : 0.2f)
								//.FillWidth_Lambda([this]() {return GetSettings()->IsMinimizeCategories() ? 0.f : 0.2f; })
								[
									SNew(SBox).WidthOverride_Lambda([this]() {return GetSettings()->IsMinimizeCategories() ? 20.f : 255.f; })
										[
...
void SLogViewerProWidgetMain::InitAsNamedTab(const FString& InTabFileName)
{
	const FString TabName = FPaths::GetBaseFilename(InTabFileName);
	NamedTabName = TabName;

	const FString NamedTabsFolderPath = SLogViewerProSettingsButton::GetDefaultNamedTabsFolderPath();
	FString NamedTabFilePath = NamedTabsFolderPath + TEXT("/") + InTabFileName;

	SLogViewerProSettingsButton::GetDefaultNamedTabsFolderPath();
	CategoryMenu->LoadCategoriesFromFile(NamedTabFilePath);
}

void SLogViewerProWidgetMain::SaveAsNamedTab(const FText& InTabName)
{
	const bool bDifferentNames = InTabName.ToString() != NamedTabName;
	if (IsNamedTab() && bDifferentNames) //Special case if we are trying to re-save named tab under different name, in that case we should duplicate existing tab and then replace with new
	{
		LogViewerProModule->RequestNewNamedTab(NamedTabName + TEXT(".json")); //@TODO better data flow for the tab name and file name, but not for this update, sorry, it takes too long already
	}

	NamedTabName = *InTabName.ToString();

	if (GetParentTab().IsValid())
	{
		GetParentTab().Pin()->SetLabel(InTabName);
	}

	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	if (DesktopPlatform == nullptr)
	{
		FMessageDialog::Open(EAppMsgType::Ok, LOCTEXT("SaveLogDialogUnsupportedError", "Saving to files is not supported on this platform!"));
		return;
	}
	...
	FPaths::NormalizeDirectoryName(NamedTabsFolderPath);
	if (!PlatformFile.DirectoryExists(*NamedTabsFolderPath))
	{
		PlatformFile.CreateDirectory(*NamedTabsFolderPath);
	}
	NamedTabsFolderPath += TEXT("/") + InTabName.ToString() + TEXT(".json");

	CategoryMenu->SaveCategoriesIntoFile(NamedTabsFolderPath);
	if (GetParentTab().IsValid())
	{
		GetParentTab().Pin()->SetOnTabClosed(SDockTab::FOnTabClosedCallback::CreateRaw(this, &SLogViewerProWidgetMain::HandleNamedTabClosed));
	}

	bNamedTabDirty = false;
}

void SLogViewerProWidgetMain::ResaveNamedTab()
{
	SaveAsNamedTab(FText::FromString(NamedTabName));
	bNamedTabDirty = false;
}

void SLogViewerProWidgetMain::HandleNamedTabClosed(TSharedRef<SDockTab> DockTab)
{
	IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
	FString NamedTabsFolderPath = SLogViewerProSettingsButton::GetDefaultNamedTabsFolderPath();
	FString FileToRemovePath = FPaths::Combine(NamedTabsFolderPath, NamedTabName + TEXT(".json"));

	IFileManager& FileManager = IFileManager::Get();
	const bool bRemoveResult = PlatformFile.DeleteFile(*FileToRemovePath);
	if (!bRemoveResult)
	{
		UE_LOG(LogTemp, Error, TEXT("LogViewer: Failed to remove named tab file %s."), *FileToRemovePath);
	}
}

void SLogViewerProWidgetMain::MinimizeCategories()
{
