...
public:
	...
	void InitAsNamedTab(const FString& TabName);
	void SaveAsNamedTab(const FText& TabName);
	void ResaveNamedTab();
	void HandleNamedTabClosed(TSharedRef<SDockTab> DockTab);
	FString GetNamedTabName() const { return NamedTabName; }

	bool IsNamedTab() const { return !NamedTabName.IsEmpty(); };
	bool IsNamedTabDirty() const { return bNamedTabDirty; };
	void MarkNamedTabDirty() { bNamedTabDirty = true; }

	void MinimizeCategories();

private:
	FString NamedTabName; //If Empty that means that's a regular tab
	bool bNamedTabDirty;
